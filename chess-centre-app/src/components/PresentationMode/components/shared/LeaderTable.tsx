/**
 * 
 * @param {Object} roundByRound
 * @param {Object} meta
 * @returns React.Component
 */
export const LeaderTable = ({ roundByRound, meta }) => {
    return (
      <div>
        <h3 className="text-2xl mb-2 -mt-4 font-semibold text-pink-900 text-center">{ meta.heading }</h3>
                ...
                return (
                  <tr key={key} className="bg-white dark:bg-gray-800">
                    ...
                    <td className="px-2 pl-4 text-left sm:px-4 py-2 whitespace-nowrap text-md font-medium text-gray-900 ">
                      { data.title && <span className="mr-1 text-yellow-400 font-bold">{data.title }</span>} {data.name}
                    </td>
